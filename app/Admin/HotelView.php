...
class HotelView implements ServiceProviderInterface {
	...
	public function render_page(): void {
		...
		$detail_hotel = get_user_by( 'id', $detail_id );
		if ( ! $detail_hotel || ! in_array( 'hotel', (array) $detail_hotel->roles, true ) ) {
			return;
		}

		$detail_code    = get_user_meta( $detail_hotel->ID, 'hotel_code', true );
		$detail_name    = get_user_meta( $detail_hotel->ID, 'hotel_name', true );
		$detail_email   = $detail_hotel->user_email;
		$detail_phone   = get_user_meta( $detail_hotel->ID, 'contact_phone', true );
		$detail_city    = get_user_meta( $detail_hotel->ID, 'city', true );
		$detail_country = get_user_meta( $detail_hotel->ID, 'country', true );
		$detail_reg     = get_user_meta( $detail_hotel->ID, 'hotel_registration', true );
		$detail_land    = get_user_meta( $detail_hotel->ID, 'hotel_landing', true );

		$created_ts = strtotime( $detail_hotel->user_registered );
		if ( ! $created_ts ) {
		...
		<h1 class="text-2xl font-bold mb-2"><?php esc_html_e( 'Hotel Detail View', 'hotel-chain' ); ?></h1>
			...
			<div class="bg-white rounded p-4 border-4 border-purple-400 bg-purple-50 mb-6" style="border: 2px solid rgb(196,196,196);">
				<div class="mb-4 pb-3 border-b-2 border-gray-300 flex items-center justify-between">
					...
					<div class="grid grid-cols-3 gap-6">
						<div class="col-span-1">
							<div class="flex items-center gap-3 mb-4">
								<div class="w-16 h-16 bg-gray-200 border-2 border-gray-300 rounded flex items-center justify-center">
								...
								<div>
									<div class="mb-1"><?php echo esc_html( $detail_name ? $detail_name : $detail_hotel->display_name ); ?></div>
									<div class="text-gray-600"><?php echo esc_html( $detail_code ); ?></div>
								</div>
							</div>
							<div class="space-y-3 mb-4">
								<div class="p-3 bg-gray-50 border-2 border-gray-300 rounded">
								...
								<div class="p-3 bg-gray-50 border-2 border-gray-300 rounded">
									...
									<span class="px-3 py-1 bg-green-200 border border-green-400 rounded text-green-900"><?php esc_html_e( 'Active', 'hotel-chain' ); ?></span>
								</div>
							</div>
							<div class="mb-4">
								...
								<div class="p-3 bg-gray-100 border-2 border-gray-300 rounded font-mono text-xs break-all" id="reg-url-text"><?php echo esc_html( $detail_reg ); ?></div>
							...
							<div class="mb-4">
								...
								<div class="p-3 bg-gray-100 border-2 border-gray-300 rounded font-mono text-xs break-all" id="land-url-text"><?php echo esc_html( $detail_land ); ?></div>
						...
						<div class="col-span-2">
							<h4 class="mb-3 pb-2 border-b-2 border-gray-300"><?php esc_html_e( 'Account Statistics', 'hotel-chain' ); ?></h4>
							<div class="grid grid-cols-3 gap-4 mb-6">
								<div class="p-3 bg-gray-50 border-2 border-gray-300 rounded text-center">
							...
							<h4 class="mb-3 pb-2 border-b-2 border-gray-300"><?php esc_html_e( 'Admin Actions', 'hotel-chain' ); ?></h4>
